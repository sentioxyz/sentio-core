...
func Test_aptosRpc(t *testing.T) {
	...
	t.Run("latestNew", func(t *testing.T) {
		tx, err := callRPC[*api.CommittedTransaction](addr, "aptos_latestNew", []any{"aptos_mainnet"})
		assert.NoError(t, err)
		assert.NotNil(t, tx)
	})
	...
	t.Run("proxy.getTransactionByVersion.404", func(t *testing.T) {
