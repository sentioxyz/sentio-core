...
func (d *SimpleDimension[SLOT]) Init(ctx context.Context) error {
	...
	if err != nil {
		return errors.Wrapf(err, "get current range failed")
	}
	var start uint64
	if !cur.IsEmpty() {
		start = *cur.End + 1
	...
	if err = d.SimpleSlotStore.Delete(ctx, rg.Range{Start: start}); err != nil {
		return errors.Wrapf(err, "clean datas in the right of current range %s failed", cur)
	}
	return nil
}
...
func (d *SimpleDimension[SLOT]) Delete(ctx context.Context, targetRange rg.Range) error {
	// Here, we need to update the range first before deleting the data from the store.
	// Because if data deletion fails, the data within the range may be incomplete, which is unacceptable.
	// Update the range first, then delete the data. If the deletion fails, either nothing will happen,
	// or there will be residual data outside the range. This is manageable, as the store will always clean up
