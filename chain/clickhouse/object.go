...
func clickhouseType(rawType reflect.Type) chx.FieldType {
	...
	case reflect.Pointer:
		panic(errors.Errorf("invalid type %s: double level pointer is invalid", rawType.String()))
	case reflect.Slice, reflect.Array:
		if nullable {
			panic(errors.Errorf("invalid type %s: array is non-nullable", rawType.String()))
		}
	...
	default:
		panic(errors.Errorf("invalid type %s: unsupported", typ.String()))
	}
}
	...
	objectx.Walk(obj, func(fields []reflect.StructField, _ reflect.Value) {
		...
		if v, has := field.Tag.Lookup(indexTag); has {
			...
			if hasGranularity {
				...
				if err != nil || granularity == 0 {
					panic(errors.Wrapf(err, "invalid granularity in the index tag value %q for the field %T.%s",
						v, obj, field.Name))
				}
			}
		...
		if vs, has := field.Tag.Lookup(projectionTag); has {
			for _, v := range strings.Split(vs, ";") {
				...
				if !ok {
					panic(errors.Errorf("invalid projection tag value %q for the field %T.%s, "+
						"should be <ProjectionName>/<FieldIndex>[;<ProjectionName>/<FieldIndex>]*", v, obj, field.Name))
				}
				index, err := strconv.ParseUint(indexStr, 10, 64)
				if err != nil {
					panic(errors.Wrapf(err, "invalid field index in the projection tag value %q for the field %T.%s",
						v, obj, field.Name))
				}
				if fn, exist := utils.GetFromK2Map(projections, projectionName, index); exist {
					panic(errors.Errorf("invalid field index in the projection tag value %q for the field %T.%s: "+
