...
func (d *ExtServerDimension) loadReceipts(ctx context.Context, st *Slot) error {
	...
	// try to use eth_getTransactionReceipt for the missing receipts
	...
	// now all receipts of the block are in receiptDict
	...
	for i, lg := range st.Logs {
		if lg.Index != uint(i) {
			return errors.Errorf("#%d log in block %d/%s has unexpected logIndex %d", i, blockNumber, blockHash, lg.Index)
		}
	}
	return nil
}
...
func (d *ExtServerDimension) loadTraces(ctx context.Context, st *Slot) error {
	...
	var count int
	gr, count = utils.RemoveInArray(gr, GethTraceBlockResult.IsEmpty)
	if count > 0 {
		logger.Warnf("got %d empty result in %s", count, method)
	}
	gr, count = utils.RemoveInArray(gr, func(trace GethTraceBlockResult) bool {
		if trace.TxHash == nil {
			return false
		}
		_, has := txIndexMap[*trace.TxHash]
		return !has
	})
	if count > 0 {
		if !d.opts.IgnoreExtraTraces {
			return errors.Errorf("has %d extra trace with unexpected transaction hash in the result of %s", count, method)
		}
		logger.Warnf("has %d extra trace with unexpected transaction hash in the result of %s", count, method)
	}
	for i, tx := range st.Block.Transactions {
		var txMissTrace bool
		if len(gr) == 0 {
			txMissTrace = true
		} else if gr[0].TxHash != nil {
			if *gr[0].TxHash == tx.Hash {
				if gr[0].Result == nil {
					txMissTrace = true
					gr = gr[1:]
				}
			} else {
				// gr[0] must be match to another transaction
				txMissTrace = true
			}
		} else if !traceTxMatch(gr[0], tx) { // gr[0].Result must be non-nil here because empty items already be removed
			// Whether gr[0] is redundant or the transaction lacks a trace is difficult to determine.
			// We can only try our best to handle it.
			// In most cases, the transaction is missing a trace, and gr[0] corresponds to another transaction.
			txMissTrace = true
		}
		if txMissTrace {
