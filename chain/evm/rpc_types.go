...
func (args *EthGetLogsArgs) UnmarshalJSON(data []byte) error {
	...
	if raw.BlockHash != nil {
		if raw.FromBlock != nil || raw.ToBlock != nil {
			...
			return errors.Errorf("cannot specify both BlockHash and FromBlock/ToBlock, choose one or the other")
		}
		...
		case []interface{}:
			for i, addr := range rawAddr {
				if strAddr, ok := addr.(string); ok {
					...
					return errors.Errorf("non-string address at index %d", i)
				}
			}
		case string:
			...
			case []interface{}:
				...
				for _, rawTopic := range topic {
					...
					if topic, ok := rawTopic.(string); ok {
						...
						return errors.Errorf("invalid topic(s)")
					}
				}
			default:
				return errors.Errorf("invalid topic(s)")
...
func (args *TraceFilterArgs) Checker() func(ParityTrace) bool {
	fromAddrSet := set.New[common.Address](args.FromAddress...)
	toAddrSet := set.New[string](utils.MapSliceNoError(args.ToAddress, strings.ToLower)...)
	return func(trace ParityTrace) bool {
		if !fromAddrSet.Empty() && (trace.Action.From == nil || !fromAddrSet.Contains(*trace.Action.From)) {
			return false
		}
		if !toAddrSet.Empty() && !toAddrSet.Contains(strings.ToLower(trace.Action.To)) {
			return false
		}
		return true
	}
...
func (r GethTraceBlockResult) IsEmpty() bool {
	return r.TxHash == nil && r.Result == nil
}
