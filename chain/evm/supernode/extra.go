...
func (s *ExtraService) EstimateBlockNumberAtDate(
	...
	if endBlock < 0 {
		if endBlock != rpc.LatestBlockNumber {
			return nil, errors.Errorf("end block number cannot be %s", endBlock.String())
		}
		en = math.MaxUint64
	}
	if startBlock < 0 {
		if startBlock != rpc.EarliestBlockNumber {
			return nil, errors.Errorf("start block number cannot be %s", startBlock.String())
		}
		sn = 0
	}
	var result *uint64
	...
	default:
		return nil, errors.Errorf("invalid mode %q, should be \"LE\" or \"GE\"", mode)
	}
		...
		func(ctx context.Context, queryRange rg.Range) ([]uint64, error) {
			...
			if mode == "GE" {
				if result == nil {
					...
					// that means the timestamp of block queryRange.End+1 is less than the target timestamp,
					// do not need to query in clickhouse
					return nil, nil
				}
				// result now is queryRange.End+1
			}
			...
			if !curRange.Include(queryRange) {
				return nil, errors.Errorf("request range %s not in scope of range store %s", queryRange, curRange)
			}
			// execute the query
			targetTime := time.UnixMilli(int64(targetTimestampMs)).UTC()
			queryResult, queryErr := s.store.QueryEstimateBlockNumberAtDate(
				ctx,
				targetTime,
				queryRange.Start,
				*queryRange.End,
			...
			// for LE mode, queryResult always be final result, because all timestamp in latest slot cache is greater
			// than target timestamp
			// for GE mode, if query result is exist then it is the final result, if query result is not exist, the result
			// in latest slot cache is final result
			if queryResult != nil {
				result = queryResult
			}
			return nil, nil
...
func (m *ProxyExtraService) getBlockHeader(ctx context.Context, blockNumber rpc.BlockNumber) (*types.Header, error) {
	...
	if err == nil && h == nil {
		return nil, errors.Errorf("header of block %s not found", blockNumber.String())
	}
	return h, err
}
