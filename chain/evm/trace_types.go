...
func populateGethTraceFromJSON(t *GethTrace, jt *gethTraceJSON) error {
	...
	if jt.Gas != "" {
		...
		if err != nil {
			return errors.Wrapf(err, "failed to decode gas")
		}
	...
	if jt.GasUsed != "" {
		...
		if err != nil {
			return errors.Wrapf(err, "failed to decode gasUsed")
		}
...
func convertOneParityToGethTrace(pr *ParityTrace) (*GethTrace, error) {
	...
	case "suicide":
		if pr.Action.RefundAddress == nil {
			return nil, errors.Errorf("refundAddress must set for suicide trace")
		}
		if pr.Action.Balance == nil {
			return nil, errors.Errorf("balance must set for suicide trace")
		}
		return &GethTrace{
	...
	case "create":
		if pr.Result.Address == nil {
			return nil, errors.Errorf("address must set for create trace")
		}
		return &GethTrace{
			Type:    "CREATE",
			From:    pr.Action.From,
	...
	return nil, errors.Errorf("unknown trace type %s", pr.Type)
}
...
func ParityToGethTrace(pr []*ParityTrace) (*GethTrace, error) {
	...
	for _, p := range pr {
		...
		for _, i := range p.TraceAddress {
			if len(curr.Calls) <= i {
				return nil, errors.Errorf("invalid trace address %v", p.TraceAddress)
			}
			curr = &curr.Calls[i]
		}
