...
func (m *ClickhouseSchemaMgr) Convert(_ context.Context, st *fuel.Slot) (clickhouse.Chunk, error) {
	...
	if err != nil {
		return clickhouse.Chunk{}, errors.Wrapf(err, "marshal header of block %d/%s failed",
			st.Block.Header.Height, st.Block.Id.String())
	}
	for i, txn := range st.Transactions {
		...
		if err != nil {
			return clickhouse.Chunk{}, errors.Wrapf(err, "marshal txn %s in block %d/%s failed",
				txn.Id.String(), st.Block.Header.Height, st.Block.Id.String())
		}
