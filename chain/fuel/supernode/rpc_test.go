...
type mockRangeStore struct {
	r rg.Range
}

func (m *mockRangeStore) Get(ctx context.Context) (rg.Range, error) {
	return m.r, nil
}

func (m *mockRangeStore) Update(ctx context.Context, operator rg.RangeOperator) (rg.Range, error) {
	m.r = operator(m.r)
	return m.r, nil
}
...
func Test_fuelRpc(t *testing.T) {
	...
	// prepare client pool
	cli := fuel.NewClientPool("client", nil)
	g.Go(func() error {
	...
	h.RegisterMiddleware(NewSuperNode(cli, sc, &rs, store)...)
	...
	t.Run("proxy.getChain.415", func(t *testing.T) {
