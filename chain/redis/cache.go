...
func (d *FixSizeSlotCache[SLOT]) GetRange(ctx context.Context) (rg.Range, error) {
	fullKey := d.keyPrefix + rangeKey
	raw, err := d.client.Get(ctx, fullKey).Result()
	if err != nil {
	...
	var cur rg.Range
	cur, err = rg.RangeParser{}.Unmarshal(bytes.NewReader([]byte(raw)))
	if err != nil {
		return rg.EmptyRange, errors.Wrapf(err, "invalid range value (%s) for key %s", raw, fullKey)
	}
	if cur.End == nil {
		return rg.EmptyRange, errors.Errorf("invalid range value (%s) for key %s: end is INF", raw, fullKey)
	}
	return cur, nil
}
...
// Here first saves the slot, then updates the range, and finally deletes the keys.
// If any step fails, at most invalid data will remain, no valid data will be lost.
// The next save operation will naturally remove the invalid data.
...
func (d *FixSizeSlotCache[SLOT]) Delete(ctx context.Context, interval rg.Range) error {
	...
	g.Go(func() error {
		for key := range ch {
			...
			n, parseErr := d.slotNumberFromKey(key)
			if parseErr != nil {
				logger.Warnfe(parseErr, "has invalid key, will be deleted")
