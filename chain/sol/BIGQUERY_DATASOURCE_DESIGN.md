...
2. **Second hop: query the heavy tables with the exact `block_timestamp` predicate.** Use the `[lo, hi]` from the first hop (full UTC days, from the day-slot index below) as the partition filter on `Transactions`/`Instructions`, narrowing the scan to the few DAY partitions involved.
...
- **Built once** with a single `GROUP BY TIMESTAMP_TRUNC(block_timestamp, DAY)` over `Blocks` (the bulk form of a per-day `MIN/MAX`), then **extended forward** one GROUP BY at a time as new complete days appear (throttled to at most once per UTC day). The extension query has **no upper time bound**; because BigQuery is not real-time, the most recent returned day may still be ingesting, so it is **dropped** — a day is admitted as complete only once a strictly-later day has data. This makes the index tolerant of ingestion lag (it never marks a not-yet-finished day "complete").
...
The MaxSlot of the **latest complete day** is the authoritative **BigQuery data boundary** — the same role the ClickHouse range store plays for the primary tier. `resolveTimeRange` maps `[from,to]` to a `block_timestamp` window by binary-searching the index — **zero BigQuery** once the index covers the range — handling non-contiguous day boundaries (skipped slots between `day(N).max` and `day(N+1).min`) and empty days. A query whose upper slot **exceeds the boundary errors** rather than being silently served from a partially-ingested tail (the caller retries, exactly as when ClickHouse is not yet synced far enough); an all-skipped gap resolves to "no data". In practice the boundary check never trips, since the BigQuery tier only ever serves slots *below* the ClickHouse range, which is far under the boundary — it is a correctness guard. The same boundary caps `EarliestProgramSlot`'s not-found watermark, so a late-arriving early instruction is never permanently missed.

### ⚠️ Cost-critical point (decided): `FindTransactions` returns only the queried programs' instructions

Measured against a busy sample day, fetching a transaction's **full** instruction set (filter `tx_signature IN @sigs` on `Instructions`) scans **~1.34 TB** — because `Instructions` is clustered by `program_id`, not by signature/slot, so the per-transaction filter cannot prune and reads the whole DAY partition. (`Transactions`, clustered by `signature`, is fine: `signature IN @sigs` prunes to ~MBs. `QueryBlock` on `Blocks` is unclustered and looked up by slot, but the day-slot index bounds it to the slot's day partition via a `block_timestamp` predicate, so it scans ~hundreds of MB rather than the whole table.)
...
- **Method wiring**:
  ...
  - **P1 (with a semantic twist)**: `EarliestProgramSlot` / `GetContractStartBlock`. **The priority direction is reversed here**: this method asks for a program's *earliest* slot, and BigQuery covers exactly the older history below ClickHouse's `minSlot`, so **BigQuery may have an earlier answer**. Semantics: search oldest data first so the first hit is the global earliest — BigQuery (slots below `range.Start`, always earlier than any ClickHouse slot), then ClickHouse, then the latest-slot cache (a brand-new program not yet synced). **`sol_getContractStartBlock` takes a `startFrom` floor** (the caller's agent start block): the result is clamped to it, and BigQuery is consulted **only when `startFrom < range.Start`** (the caller wants — and is allowed — history below the ClickHouse range). A caller floored at or above the range start (e.g. one without BigQuery access, whose floor is the range start; see `firstSlot` below) therefore never triggers a BigQuery query, which also avoids a permission-denied error for it. (Searching BigQuery first — rather than only when ClickHouse shows the program at its lower bound — costs one extra archival scan for an in-window program, but is simpler and also catches a program that appeared in old history, went quiet across the lower bound, then reappeared.)
  - **`firstSlot` in `sol_getLatestHeader`**: the response (a lightweight `SimpleBlock` header + `firstSlot` + `apiVersion`) carries the caller's earliest indexable slot — `0` when the caller may use the BigQuery tier (`bq.Store.CheckPermission` passes), otherwise the ClickHouse `range.Start`. The driver clamps the agent's start block to it, so a caller without BigQuery access does not try to backfill below the ClickHouse range. `DriverVersion==4 && SupportsSuperNode` does **not** by itself grant BigQuery access — the per-caller `PermissionChecker` decides — so the floor is computed per request, not from the driver version.
...
| 10 | `resolveTimeRange` day-slot index | back slot→timestamp resolution with a persistent per-UTC-day min/max-slot index (one Redis key, built/extended via `GROUP BY` day), removing the ~6 GB `Blocks` scan per call; the most-recent (possibly still-ingesting) day is dropped, so the latest complete day's MaxSlot is the BigQuery data boundary; a query above the boundary errors (like the ClickHouse range store); the boundary also caps `EarliestProgramSlot`'s not-found watermark | §5 "Day-slot index" |
