...
func TypeTagFromString(s string) (*TypeTag, error) {
	...
	default:
		...
		return nil, errors.Errorf("invalid type tag: %s", s)
	}
}
...
func (s TypeTag) MarshalBCS() ([]byte, error) {
	...
	if s.Bool {
		...
		return nil, errors.Errorf("invalid TypeTag: %v", s)
	}
...
func (s *TypeTag) UnmarshalBCS(r io.Reader) (int, error) {
	...
	case 10:
		...
		s.U256 = true
	default:
		return 0, errors.Errorf("invalid enum id %d", enumID)
	}
	if serde.Trace {
...
func (s *TypeTag) SerializeBCS(v json.RawMessage) ([]byte, error) {
	...
	if s.Bool {
		...
		if !ok {
			return nil, errors.Errorf("invalid u128 value: %s", v)
		}
		...
		_, ok := b.SetString(decodeString(), 10)
		if !ok {
			return nil, errors.Errorf("invalid u256 value: %s", v)
		}
...
func (s *PureValue) RawBytes() ([]byte, error) {
	...
	if s.json == nil {
		return nil, errors.Errorf("no type information provided")
	}
	var err error
...
func (s *PureValue) UnmarshalJSON(data []byte) error {
	...
	if j.Type != "pure" {
		return errors.Errorf("invalid pure value %s", j.Type)
	}
	s.json = &j
	return nil
}
