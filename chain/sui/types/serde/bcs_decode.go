...
func (d *Decoder) Decode(v any) error {
	...
	if reflectValue.Kind() != reflect.Pointer || reflectValue.IsNil() {
		return errors.Errorf("not a pointer or nil pointer")
	}
...
func (d *Decoder) decode(v reflect.Value) error {
	...
	case reflect.Interface:
		if v.IsNil() {
			debug.PrintStack()
			return errors.Errorf("cannot decode into nil interface")
		}
...
func (d *Decoder) decodeVanilla(v reflect.Value) error {
	...
	if !v.CanSet() {
		return errors.Errorf("cannot change value of kind %s", kind.String())
	}
	...
	case reflect.Bool:
		t, err := d.readByte()
		if err != nil {
			return errors.Wrapf(err, "cannot read boolean")
		}
	...
	default:
		return errors.Errorf("unsupported vanilla decoding type: %s", kind.String())
	}
}
...
func (d *Decoder) decodeString(v reflect.Value) error {
	...
	if size != read {
		return errors.Errorf("wrong number of bytes read for []byte, want: %d, got %d", size, read)
	}
...
func (d *Decoder) decodeEnum(v reflect.Value) error {
	if v.Kind() != reflect.Struct {
		return errors.Errorf("only support struct for Enum, got %s", v.Kind().String())
	}
...
func ReadByteSlice(r io.Reader) ([]byte, error) {
	...
	if size != read {
		return nil, errors.Errorf("wrong number of bytes read for []byte, want: %d, got %d", size, read)
	}
