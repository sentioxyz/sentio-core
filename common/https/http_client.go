...
var (
	defaultMaxIdleConnsPerHost uint64
	defaultMaxIdleConns        uint64
	defaultTimeout             time.Duration
)

func init() {
	defaultMaxIdleConnsPerHost = envconf.LoadUInt64("MAX_IDLE_CONNS_PER_HOST", 2000)
	defaultMaxIdleConns = envconf.LoadUInt64("MAX_IDLE_CONNS", 2000)
	defaultTimeout = envconf.LoadDuration("DEFAULT_HTTP_TIMEOUT", time.Minute*10)
