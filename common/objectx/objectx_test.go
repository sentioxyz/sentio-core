...
func Test_CollectTagValue(t *testing.T) {
	...
	assert.PanicsWithError(t, "nil pointer", func() {
		_ = CollectTagValue((*ObjectA)(nil), "xxx")
	})
...
func Test_UnexportedFieldsSkipped(t *testing.T) {
	type Obj struct {
		Exported   string `xxx:"e"`
		unexported string `xxx:"u"`
	}
	o := Obj{Exported: "yes", unexported: "no"}
	assert.Equal(t, []string{"e"}, CollectTagValue(o, "xxx", HasTag("xxx")))
	assert.Equal(t, []any{"yes"}, CollectFieldValues(o, HasTag("xxx")))
}
