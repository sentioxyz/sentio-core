...
func TestCompileUpdateExp_errors(t *testing.T) {
	...
	cases := []struct {
		...
		{"propA1", "concat()", "unsupported operator 'concat' at expression[0..5] with 0 arguments"},
		{"propA1", "concat(propA1, propD1)", "argument #2 is integer, expect string"},
		{"propA1", "concat(propA1, true)", "argument #2 is boolean, expect string"},
		{"propD1", "concat(propA1, 'x')", "expression result is string but field EntityD.propD1 is integer"},
		{"propA1", "toString()", "unsupported operator 'toString' at expression[0..7] with 0 arguments"},
		{"propA1", "toString(1, 2)", "with 2 arguments"},
		{"propD1", "toString(propD1)", "expression result is string but field EntityD.propD1 is integer"},
		{"propA1", "propA1 + 'x'", "argument #1 is string, expect number"},
...
func TestCompileUpdateExp_kinds(t *testing.T) {
	...
	cases := []struct {
		...
		{"propA1", "concat(propA1, '-', toString(propD1))", kindString, []string{"propA1", "propD1"}},
		{"propA1", "concat(null)", kindString, nil},
		{"propA1", "toString(null)", kindString, nil},
		{"propA1", "toString(propC1)", kindString, []string{"propC1"}},
		{"propA1", "toString(propF1 / 3)", kindString, []string{"propF1"}},
		{"propC1", "toString(propD1) = '10'", kindBool, []string{"propD1"}},
...
func TestCompiledExp_eval(t *testing.T) {
	...
	cases := []struct {
		...
		// concat / toString
		{"propA1", "concat(propA1, '-', propG1)", row, expString("abc-BBB")},
		{"propA1", "concat(propA1)", row, expString("abc")},
		{"propA1", "concat(propA1, propA2)", row, expNull},
		{"propA1", "concat(propA1, coalesce(propA2, ''))", row, expString("abc")},
		{"propA1", "toString(propD1)", row, expString("10")},
		{"propA1", "toString(propE1 * 1000000000000)", row, expString("1000000000000000")},
		{"propA1", "toString(propF1)", row, expString("1.5")},
		{"propA1", "toString(propF2 * 2)", row, expString("5")},
		{"propA1", "toString(propI1)", row, expString("0.25")},
		{"propA1", "toString(propD1 / 4)", row, expString("2.5")},
		{"propA1", "toString(propD1 div 4)", row, expString("2")},
		{"propA1", "toString(propH1)", row, expString("1700000000000000")},
		{"propA1", "toString(propC1)", row, expString("true")},
		{"propA1", "toString(propC2)", row, expString("false")},
		{"propA1", "toString(propA1)", row, expString("abc")},
		{"propA1", "toString(propB1)", row, expString("0x0a")},
		{"propA1", "toString(propD2)", row, expNull},
		{"propA1", "toString(propD1)", none, expNull},
		{"propA1", "toString(-1.50)", row, expString("-1.5")},
		{"propA1", "concat('id-', toString(propD1 + 1), if(propC1, '-on', '-off'))", row, expString("id-11-on")},
	}
	for i, c := range cases {
		field := e.GetFieldByName(c.field)
		compiled, err := compileUpdateExp(e, field, c.exp)
