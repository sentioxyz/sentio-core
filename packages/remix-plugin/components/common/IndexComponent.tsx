...
import { IRemixApi, CompilationResult as CompilationResultType, RemixTxEvent } from '@remixproject/plugin-api'
...
import { useGlobalCompilationResultStore, useGlobalContractStore } from '@/lib/use-global-store'
...
export const IndexComponent = () => {
  ...
  const [network, setNetwork] = useState<string>(EthChainIds[EthChainName.ETHEREUM])
        ...
        case 'ContractDefinition': {
          ...
          childrenNodes.forEach((childNode) => {
            if (childNode.nodeType === 'FunctionDefinition') {
              if (childNode.visibility === 'public' && childNode.kind !== 'constructor') {
                const functionSelector = childNode.functionSelector
                const name = childNode.name
                const functionAbi = contracts[node.name]?.abi.find((abiItem) => {
                  try {
  ...
  useEffect(() => {
    async function load() {
      ...
      client.solidity.on(
        'compilationFinished',
        (fn: string, source: any, languageVersion: string, data: CompilationResultType) => {
          parseResult(data)
      ...
      client.udapp.on('newTransaction', (tx: RemixTxEvent) => {
        setTransactions((preValue) => {
          if (preValue.filter((item) => item.hash === tx.hash).length > 0) return preValue
          return [tx, ...preValue]
        })
      })
    }
        ...
        <div className="bg-light sticky top-0 z-[1] space-y-2 px-2 py-2 shadow-sm">
          <Collapsible className="space-y-2">
            <div className="flex w-full flex-wrap items-center gap-2">
              {fileName ? (
                <>
                  <div className="flex flex-1 items-center gap-6 overflow-hidden py-2" title={fileName}>
                    <div className="flex-0 inline-flex items-center gap-1">
                      <i className="fa-regular fa-file-code text-dark h-4 w-4"></i>
                      <span className="text-dark text-sm font-semibold">{fileInfo?.name}</span>
                    </div>
                    <span className="shrink-1 ml-auto grow-0 truncate text-xs">{fileInfo.path}</span>
                  </div>
                </>
              ) : (
              ...
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p className="max-w-[200px]">Set your API keys, target project and setting chain network</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
            <CollapsibleContent className="space-y-2">
              <SettingContent label="API Key" placeholder="Input your Sentio API Key" storageId="api-key" />
              <SettingContent
                label="Project Owner"
                placeholder="Input your project owner here"
                storageId="projectOwner"
              />
              <SettingContent label="Project Slug" placeholder="Input your project slug here" storageId="projectSlug" />
              <div className="flex items-center gap-2">
                <label className="w-20 shrink-0 pr-2 pt-2 text-right text-xs font-medium">Network: </label>
                <Select onValueChange={setNetwork} value={network}>
                  ...
                  <SelectContent>
                    <SelectGroup>
                      ...
                      {[EthChainName.ETHEREUM, EthChainName.POLYGON, EthChainName.MOONBEAM, EthChainName.ASTAR].map(
                        (networkName) => (
                          <SelectItem value={EthChainIds[networkName]} key={networkName}>
              ...
              <TabsContent value="overview">
                <ul className="">
                  {functions.map((item, index) => {
                    return (
                      <li key={index} className="group border-b px-4 py-2 text-sm">
                          ...
                          <div className="inline-flex flex-1 flex-wrap items-center justify-between gap-2">
                            <div
                              ...
                              onClick={async () => {
                                ...
                                const lineRange = sourceMappingRef.current?.srcToLineColumnRange(item.src)
                            ...
                            >
                              <span className="text-dark font-semibold">{item.functionName || `${item.name}()`}</span>
                            </div>
                            <div className="inline-flex items-center gap-2">
                              <label className="mb-0 font-mono text-xs">{item.functionSelector}</label>
                              <CopyButton
                          ...
                          <button
                            ...
                            onClick={() => {
                              setShowSearchFunction((preValue) => {
                                if (preValue === item.functionSelector) return ''
                        ...
                        {item.functionSelector && showSearchFunction === item.functionSelector ? (
                          <RelatedTransactions open={true} chainId={network} methodSignature={item.functionSelector} />
                        ) : null}
                      </li>
                    )
                  })}
