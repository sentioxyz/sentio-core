...
export const TxIdentifierContext = createContext<
  'txId.txHash' | 'txId.simulationId' | 'txId.bundleId'
>('txId.txHash')
...
export const BundleSetIndexContext = createContext<
  ((idx: number) => void) | undefined
>(undefined)
export const ChainIdentifierContext = createContext<
  'chainSpec.chainId' | 'chainSpec.forkId'
>('chainSpec.chainId')
export const SimShareIdContext = createContext<string | undefined>(undefined)
export const PriceFetcherContext = createContext(async function priceFetcher(
  params: any
) {
  return {} as any
})
