...
type FilterFunctionSuite struct {
	testsuite.Suite
}

func Test_RunFilterFunctionSuite(t *testing.T) {
	...
	suite.Run(t, new(FilterFunctionSuite))
}

func (s *FilterFunctionSuite) Test_Filter() {
	...
	sql, err := NewFilterFunction(timeseries.Meta{
		Name: "Transfer",
		Type: timeseries.MetaTypeGauge,
	}, s.Store).Filter().WithSelector(sel).WithLabels([]string{"meta.chain", "from"}).Generate()
	s.Nil(err)
	s.Check(testsuite.GetCurrentFunctionName(), sql)
}

func (s *FilterFunctionSuite) Test_Filter_WithTimeRange() {
	sel := selector.NewSelector(timeseries.Meta{
		Name: "Transfer",
		Type: timeseries.MetaTypeGauge,
		Fields: map[string]timeseries.Field{
	...
	sql, err := NewFilterFunction(timeseries.Meta{
		Name: "Transfer",
		Type: timeseries.MetaTypeGauge,
	}, s.Store).Filter().WithSelector(sel).WithLabels([]string{"meta.chain", "from"}).WithTimeRange(mock.NewTimeRange()).Generate()
	s.Nil(err)
	s.Check(testsuite.GetCurrentFunctionName(), sql)
}

func (s *FilterFunctionSuite) Test_Filter_NoSelector() {
	sql, err := NewFilterFunction(timeseries.Meta{
		Name: "Transfer",
		Type: timeseries.MetaTypeGauge,
	}, s.Store).Filter().Generate()
	s.Nil(err)
	s.Check(testsuite.GetCurrentFunctionName(), sql)
}
